...
The DOI archive is not retroactively relicensed or changed by later commits on `main`. At the present handoff, the GitHub repository may remain private while its contents and CI are verified; the Zenodo record is already the live public citation target.
...
Current maintained-branch local QA: **47 tests passed** and all five legacy stress CSVs reproduced **byte-exactly**. GitHub push, clean-checkout CI, and public visibility remain pending. The `1.1.0-candidate` model remains unpromoted.
